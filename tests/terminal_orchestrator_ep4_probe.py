...
def check_closure(workspace, *, terminal_runs: int = 1) -> None:
    if terminal_runs <= 0:
        raise ValueError("terminal_runs must be positive")
    expected = {
        "in_use": 0,
        ...
        # The input rendezvous uses a monotonic epoch.  Each terminal
        # transaction contributes two EP-wide arrivals and prepare stores the
        # resulting target in input_expected_scratch; neither value is reset
        # between successful leases.
        "barrier_target": terminal_runs * 2 * EP_SIZE,
        "input_expected_scratch": terminal_runs * 2 * EP_SIZE,
    }
    observed = {
...
def main() -> int:
    ...
    try:
        ...
        # Exercise the exact transaction used by SGLang direct quantization:
        # validate/acquire first, populate the symmetric input buffers while
        # the lease is held, then enter the no-copy preloaded terminal path.
        # copy_ models the caller-owned quant kernel here; the production
        # runner replaces only these two writes with its numerically identical
        # out-variant quant launch.
        preloaded_output = torch.full_like(output, float("nan"))
        acquire_megakernel_fp8_block_from_topk_lease(
            workspace,
            config,
            w13,
        ...
        torch.cuda.synchronize(device)
        require_exact("preloaded_output", output, preloaded_output)
        check_schedule(workspace, expected_schedule, peer_topk_ids)
        check_closure(workspace, terminal_runs=2)
        ...
        if rank == 0:
            print(
                ...
                "|output=bitwise_exact|preloaded=bitwise_exact"
                "|inactive=untouched"
